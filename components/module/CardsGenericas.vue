...
{
  type Classes = ('has-1-item' | 'is-2-cols' | 'has-2-items' | 'has-3-items' | 'has-swiper');
  const list: Classes[] = [];

  const amount = prop.m.props.cards.value.length;
  if (amount == 1) list.push('has-1-item');
  else if (amount == 2) list.push('has-2-items');
  else if (amount == 3) list.push('has-3-items', 'has-swiper');
              ...
              <div class="inner">
                <div class="top">
                  <div class="image" v-if="c.media.type != 'none'">
                    ...
                    :is="c.tag" class="image-div" :href="c.link"
                    >
                      <BuilderImage />
                    </component>
                  </div>
